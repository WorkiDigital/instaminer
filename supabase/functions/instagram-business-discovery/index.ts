...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { username, after } = await req.json().catch(() => ({}));
    ...
    let mediaQuery = "media.limit(50)";
    if (after) {
      mediaQuery += `.after(${after})`;
    }
    mediaQuery += "{id,caption,like_count,comments_count,media_type,media_product_type,permalink,timestamp,thumbnail_url}";
    ...
    try {
      ...
      return json({ error: "Erro interno ao acessar a API: " + err.message }, 500);
    }
    ...
    if (!response.ok || !payload.business_discovery) {
      ...
      if (errCode === 100 && errMsg.includes("business_discovery")) {
        return json({
          error: "Business Discovery nao disponivel. A conta alvo pode ser pessoal ou estar com o perfil privado.",
    ...
    return json({
      ...
      paging: profile.media?.paging || null,
    });
  } catch (globalErr: any) {
    console.error("Global error in function:", globalErr);
    return json({ error: "Erro interno no servidor: " + (globalErr.message || String(globalErr)) }, 500);
  }
});
